...
docker volume create tunnel-stack_jellyfin_config > /dev/null 2>&1 || true
docker volume create tunnel-stack_linkding_data > /dev/null 2>&1 || true
...
chmod +x "$SCRIPT_DIR/backup-jellyfin.sh"
...
chmod +x "$SCRIPT_DIR/restore-immich.sh"
...
(
  crontab -u root -l 2>/dev/null | grep -v "backup-vaultwarden\|backup-pihole\|backup-jellyfin\|backup-linkding\|backup-homeassistant\|backup-immich"
  # Use absolute paths — cron runs as root where ~ expands to /root/, not the user's home
  ...
  echo "0 2 * * * bash ${SCRIPT_DIR}/backup-immich.sh >> ${SCRIPT_DIR}/logs/backup-immich.log 2>&1"
) | crontab -u root -
